...
export default function TraineeRegister() {
  ...
  async function handleSubmit(event) {
    ...
    if (isValidPassword) {
      const response = await fetch('/api/auth/trainee-register', {
        method: 'POST',
      ...
      if (result.code == 201) {
        ...
        toast({
          title: 'Trainee account created successfully.',
          status: 'success',
          position: 'top-right',
          duration: 5000,
          isClosable: true,
    ...
    <Stack maxH={'83.5vh'} direction={{ base: 'column', md: 'row' }}>
      ...
      <Flex p={8} flex={1} align={'center'} justify={'center'}>
        <Stack spacing={4} w={'full'} maxW={'lg'}>
          <Heading fontSize={'4xl'}>Register as a Trainee</Heading>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <Stack pt={'3rem'} align={'center'}>
              <Flex gap={'10px'}>
                <Text> Too buff to gatekeep? </Text>
                <Text color={'blue.400'} textDecorationLine={'underline'}>
                  <Link href={'/trainer-register'}>Register as a Trainer</Link>
                </Text>
              </Flex>
            </Stack>
          </form>
